  ...
  return (options) => {
    ...
    _.each(helper('core:pkgs')(p => {
      ...
      if (fs.existsSync(dir)) {
        pkg.trace('Template » Serve -> %s:%s', p.id, helper('core:makeRelDir')(dir, null, ''))
        dirs.push(dir)
      }
    })
    const rootTpl = path.join(cuk.dir.root, 'cuks', 'view', 'template')
    dirs.unshift(rootTpl)
    pkg.trace('Template » Serve -> %s:%s', 'root', helper('core:makeRelDir')(rootTpl, null, ''))
